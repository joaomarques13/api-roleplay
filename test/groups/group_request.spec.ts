...
test.group('Group Request', (group) => {
  ...
  test('it should return 404 when provinding an unexisting group for rejection', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
  ...
  test('it should return 404 when provinding an unexisting group request for rejection', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
    ...
    const response = await supertest(BASE_URL)
      .delete(`/groups/${group.id}/requests/123t`)
      .set('Authorization', `Bearer ${token}`)
      .expect(404)
